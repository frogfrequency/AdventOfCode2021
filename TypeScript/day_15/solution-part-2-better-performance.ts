...
function giveAdjacents(coords: number[]): number[][] {
    ...
    if (y != 0 && map[y-1][x] > 0) { adjacentsCoordsArr.push([y - 1, x]) }; // if statments prevent code to try and access non-existent arrays or idxs like arr[-1]
    if (x != map[0].length - 1  && map[y][x+1] > 0) { adjacentsCoordsArr.push([y, x + 1]) };
    if (y != map.length - 1 && map[y+1][x] > 0) { adjacentsCoordsArr.push([y + 1, x]) };
    if (x != 0 && map[y][x-1] > 0) { adjacentsCoordsArr.push([y, x - 1]) };
