...
function giveAddedLines(lineA: string, lineB: string): string {
    return '[' + lineA + ',' + lineB + ']'
}

function setLargestMagnitude(): void {
    for (let i = 0; i < inputLinesArr.length - 1; i++) {
        for (let j = i + 1; j < inputLinesArr.length; j++) {
            let firstCombination: string = giveAddedLines(inputLinesArr[i], inputLinesArr[j]);
            let secondCombination: string = giveAddedLines(inputLinesArr[j], inputLinesArr[i]);

            let firstMagnitude: number = giveMagnitude(giveReducedLine(firstCombination));
            let secondMagnitude: number = giveMagnitude(giveReducedLine(secondCombination));
            if (largestMagnitude < firstMagnitude) {
                largestMagnitude = firstMagnitude;
            }
            if (largestMagnitude < secondMagnitude) {
...
console.log('the largest Magnitude is: ' + largestMagnitude);
