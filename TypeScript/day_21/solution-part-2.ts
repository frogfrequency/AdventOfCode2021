...
let playerPositions: number[] = [input[0], input[1]]; 
...
function movePawn(startingPosition: number, sumOfThreeRolls: number) {
    let landingSpace: number = (startingPosition + sumOfThreeRolls) % 10
    if ( landingSpace === 0 ) {
        return 10
    } else {
        return landingSpace
...
function playGame(winningScore: number) {
