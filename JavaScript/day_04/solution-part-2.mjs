...
function playGame(draws) {
    for (let i = 0; i < draws.length; i++) {
        ...
        let cardIdxsIncludingABingo = [];

        for (let j = 0; j<cards.length; j++) { // looping through cards and collecting card idxs that include Bingo
            let cardsBingoScore = giveCardsBingoScore(cards[j]);
            if (cardsBingoScore != -1) {
                if (cards.length === 1) {
                    console.log(`last card finally got a Bingo!`);
                    console.log(`It's sum of unmarked numbers (${giveRemainingScore(cards[0])}) multiplied by last number (${currentNumber}) = ${giveRemainingScore(cards[0]) * currentNumber}`);
                }
                cardIdxsIncludingABingo.unshift(j); // making order of idxs from highest to lowest so that removing a card doesnt change idxs of remaining cards 
            }
        }

        for (let j = 0; j<cardIdxsIncludingABingo.length; j++) { 
            cards.splice(cardIdxsIncludingABingo[j], 1);
        }
...
function giveCardsBingoScore(card) { // return -1 if no Bingo found in this cards. otherwise returns sum of remaining numbers in card
    let bingoScore = -1;    
        for (let i = 0; i < 5; i++) {
